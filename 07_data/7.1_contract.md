...
| 安全 | 谁能读取，哪些字段需要脱敏，允许用途和禁止用途是什么，代理/服务账号代表谁执行 | 权限组、列级策略、allowed_use、delegated subject、审计日志 |
