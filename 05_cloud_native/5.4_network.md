...
现场还必须检查是否依赖 `ingress-nginx`。Kubernetes SIG Network 和 Security Response Committee 于 2026-03-24 宣布 Ingress NGINX 项目退休；退休后不再发布 bugfix、安全补丁或其他更新。仍使用它的客户应规划迁移到 Gateway API 实现或其他受维护入口控制器，而不是只把现有 Ingress 规则原样保留。迁移清单至少要盘点注解、snippet、rewrite、auth、rate limit、TLS、WAF、body size、timeout、错误页、访问日志字段和回滚入口；其中 snippet 与控制器私有注解通常不能机械翻译成 Gateway API 对象，需要逐条确认替代实现和风险接受。
