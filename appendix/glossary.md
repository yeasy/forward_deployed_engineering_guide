...
AI FDE 指把模型、代理和工具调用纳入前线部署工程闭环。它不是让 AI 取代 FDE，而是让 AI 参与检索、生成、变更草拟、测试、运行手册更新和日志分析等动作；FDE 仍负责定义身份、权限、预算、环境、回滚和证据边界。
...
ML-BOM 是机器学习物料清单，用来描述模型、数据集、训练或微调过程、依赖、环境和模型透明度信息；客户工具链支持时，应优先使用 CycloneDX 等标准化 ML-BOM 表达模型资产。AIBOM 可作为项目内部的 AI 资产清单别名，用于补充提示词、评测集、工具、RAG 索引、模型路由和运行策略，但不应替代标准 ML-BOM。
...
证据包是交付、发布、变更、合规或客户接管过程中用于证明系统可运行、可审计、可回滚和可移交的一组材料。发布证据包绑定某次 release 的制品、SBOM/ML-BOM、签名、provenance、测试和审批；变更证据包绑定某次配置、模型、数据或权限变化；项目/合规证据包面向审计和客户验收；交付包则包含客户接管所需的运行手册、权限流程、培训和开放风险。
...
产品化指把现场交付中已验证、可复用且具有业务语义的能力沉淀为有路线图、版本、支持承诺和长期 owner 的产品能力。产品化不同于平台化：平台化强调交付底座复用，产品化强调可被用户理解、采购、采用和持续演进的领域能力。
...
MCP 是 Model Context Protocol 的缩写，由 Anthropic 在 2024 年开源；本书固定引用 [Model Context Protocol 2026-07-28 规范](https://modelcontextprotocol.io/specification/2026-07-28)，这是当前修订版；上一修订版 `2025-11-25` 的初始化握手与协议级会话已被移除，协议转为无状态，Roots、Sampling、Logging 转入至少十二个月的弃用期（仍可用，新实现不建议采用）。钉版是刻意为之，为让全书引用指向同一份规范。它为 LLM 应用与外部数据源、工具之间定义了标准化接口，使工具可被支持 MCP 的客户端、host 或 agent 框架复用，而不必为每个模型 SDK 单独适配。Linux Foundation 公开信息显示 MCP 已进入 Agentic AI Foundation；协议治理中立化不等于生产安全默认成立，MCP server 仍需按供应链资产和工具权限治理。
...
按 [NIST SP 800-207](https://csrc.nist.gov/pubs/sp/800/207/final) 的定义，零信任不对任何资产或用户账户仅凭其所处的物理或网络位置授予隐式信任，访问决策按请求、按最小权限动态作出。业界常用“永不信任、始终验证”概括这一思路，但该措辞并不出自 SP 800-207。它要求每次访问都基于身份、设备、上下文与策略动态判断，而不依赖网络边界。FDE 在现场交付时将其落到身份、权限、审批、审计与最小可执行权限。
