...
PUBLICATION = ("ci.yaml", "preview-pdf.yml", "auto-release.yml")
...
upload = ["release", "upload", "preview-pdf", "dist/forward_deployed_engineering_guide.pdf", "dist/forward_deployed_engineering_guide.html", "dist/SHA256SUMS", "--clobber"]
...
class ReleaseWorkflowTests(unittest.TestCase):
    ...
    def test_all_actions_are_full_sha_pinned_and_permissions_default_deny(self):
        failures = []
    ...
    def test_publication_builds_are_read_only_complete_and_fail_closed(self):
    ...
    def test_release_is_attested_and_write_permissions_are_isolated(self):
        ...
        self.assertRegex(release, r"(?s)subject-path:.*?\.pdf.*?\.html.*?SHA256SUMS")
        self.assertRegex(release, r"(?s)files:.*?\.pdf.*?\.html.*?SHA256SUMS")
        self.assertIn("fail_on_unmatched_files: true", release)
        preview = self.text("preview-pdf.yml")
        self.assertIn("contents: write", job_text(preview, "publish"))
        self.assertNotIn("contents: write", job_text(preview, "build"))
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for name in step_names(text):
                ...
                if result.returncode:
                    break
            if result is None:
                result = subprocess.CompletedProcess([], 127, "", "preview publish steps missing")
            commands = [json.loads(line) for line in log.read_text().splitlines()] if log.exists() else []
            return result, commands

    def test_preview_mutation_order_exact_argv_and_not_found_semantics(self):
        text = self.text("preview-pdf.yml")
        ...
        self.assertEqual(commands[-1][-4:], ["dist/forward_deployed_engineering_guide.pdf", "dist/forward_deployed_engineering_guide.html", "dist/SHA256SUMS", "--clobber"])
    ...
    def test_preview_fails_closed_for_ambiguous_rc_and_invalid_inputs(self):
