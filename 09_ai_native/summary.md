...
本章的核心结论有五点。第一，AI 用例要从业务动作反推，像售后工单 Agent 一样先界定流程、角色、数据源、指标和不可自动化动作。第二，LLM 应用架构必须包含上下文构造、工具调用、输出校验、评估观测和审计，而不是把提示词当作全部系统。第三，RAG 是企业知识接入模式，不是正确性的保证；检索、权限、引用、刷新和反馈共同决定质量。第四，Agent 工作流要先受控再自主，优先使用 routing、顺序工作流、orchestrator-workers 和 evaluator-optimizer 等可解释模式。第五，人机协同要把操作边界写进系统，用人工确认、回滚、审计和反馈闭环控制风险。
